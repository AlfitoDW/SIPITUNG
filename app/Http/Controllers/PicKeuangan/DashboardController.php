...
class DashboardController extends Controller
{
    public function index(): Response
    {
        ...
        $nilaiVerifikasi = $baseQuery->clone()->where('status', 'ppk_approved')->sum('total_anggaran');
        ...
        $pipeline = $tahunId
            ...
            : ['ppk_approved' => 0, 'pic_approved' => 0, 'dicairkan' => 0];

        // ─── Tugas: ppk_approved yang menunggu verifikasi PIC ──────────────
        ...
        $tugasHariIni = $tugasHariIni->map(function ($pd) {
            $pd->hari_menunggu = $pd->ppk_approved_at ? Carbon::parse($pd->ppk_approved_at)->diffInDays(now()) : 0;
            return $pd;
        });
